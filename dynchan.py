...
async def help(ctx):
    ...
    embed.add_field(name="remove [name]", value="[Admin Only] Remove an autochannel using its specific name.", inline=False)
...
async def remove(ctx, arg):
    ...
    await checking_delete(ctx.guild.get_channel(cat_id), arg)
...
async def checking_delete(category, name):
    for i in category.channels:
        if i.name[:i.name.find("#")-1] == name:
        #if i.members == 0:
